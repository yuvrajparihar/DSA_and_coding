...
void prefix(vector<ll> &hash,vector<ll> &inv,int n,string hay)
{
    int p=1;
    int hashVal=0;
    for(int i=0;i<n;i++)
...
int main()
{
    ...
    {  
        ...
        cin>>needle>>hay;
        int n=hay.length();
        vector<ll> hash(100001),inv(100001);
        prefix(hash,inv,n,hay);
        ...
        {   
            ...
            if(i==0)
            {
               val= (hash[i]*1ll*inv[i]) %mod;
            }
            else
            {     
                 val= (((mod+hash[i+m-1]-hash[i-1])%mod)*1ll*inv[i])%mod ;
                            
                // cout<<val<<" "<<((mod+hash[i+m-1]-hash[i-1])%mod)<<" "<<inv[i]<<endl;
